...
func _ready() -> void:
	...
	parent.add_excluded_object(player.get_rid())
	ray.add_exception(player)
	ray2.add_exception(player)
	ray3.add_exception(player)
	ray4.add_exception(player)
	ray5.add_exception(player)
...
func _process(delta: float) -> void:
	if follow != player and follow != null:
		...
		if Vector3.UP.distance_to(parent.global_position.direction_to(follow.global_position)) > 0.015:
			...
			parent.look_at_from_position(parent.global_position,follow.global_position,Vector3.UP)
